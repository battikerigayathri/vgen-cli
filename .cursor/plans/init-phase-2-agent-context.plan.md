...
todos:
  - id: skill-index-and-router
    content: "resmed_resmate-cli — Update templates/workspace/.cursor/skills/resmate-use-case/SKILL.md with new router rows, short ID lifecycle section, and push-wire-re-push checklist"
    status: pending
  - id: id-lifecycle-doc
    content: "resmed_resmate-cli — Create templates/workspace/.cursor/skills/resmate-use-case/docs/id-lifecycle.md documenting omit -> push -> write-back lifecycle and anti-patterns"
    status: pending
  - id: push-pull-wire-doc
    content: "resmed_resmate-cli — Create templates/workspace/.cursor/skills/resmate-use-case/docs/push-pull-wire.md documenting phase_order, binding rules (slug vs Mongo id), and push-all wiring limits"
    status: pending
  - id: cli-commands-doc-update
    content: "resmed_resmate-cli — Expand templates/workspace/.cursor/skills/resmate-use-case/docs/cli-commands.md with push/pull write-back and wiring details"
    status: pending
  - id: rule-links-retarget
    content: "resmed_resmate-cli — Retarget broken platform/ and cli/ relative links in templates/workspace/.cursor/rules/resmate-use-cases.mdc to skill docs"
    status: pending
  - id: agents-md-update
    content: "resmed_resmate-cli — Update templates/workspace/AGENTS.md to point push order and ID lifecycle instructions directly to the skill docs"
    status: pending
  - id: examples-id-cleanup
    content: "resmed_resmate-cli — Strip or annotate invented/hardcoded Mongo ObjectIds from templates/workspace/examples/** YAML files"
    status: pending
isProject: false
---
...
|-----------------------------------------|--------------|
| `templates/workspace/.cursor/skills/resmate-use-case/SKILL.md` | Changes in `teemo`, `pr-agent-v2`, `resmedai-core-framework` |
| `templates/workspace/.cursor/skills/resmate-use-case/docs/id-lifecycle.md` (NEW) | SDK response pattern cookbooks (Phase 3) |
| `templates/workspace/.cursor/skills/resmate-use-case/docs/push-pull-wire.md` (NEW) | HITL resume message contracts (Phase 3) |
| `templates/workspace/.cursor/skills/resmate-use-case/docs/cli-commands.md` | Any CLI binary or Rust code changes (unless needed for template packing) |
| `templates/workspace/.cursor/rules/resmate-use-cases.mdc` | CLI-side push-validation warnings for invented IDs (Phase 5) |
...
| 4 | **Push-All Wiring Limit** | `resmate push-all` orders by phase but **does not** auto-wire IDs into dependents. Authors/agents must manually wire written-back IDs after pushing each layer, then re-push the dependent layers. |
...
- `resmate-use-cases.mdc` contains numerous broken relative links pointing to non-existent `../../platform/` and `../../tools/` directories.
...
### 1. Update Skill Router — `templates/workspace/.cursor/skills/resmate-use-case/SKILL.md`
...
### 2. Create ID Lifecycle Document — `templates/workspace/.cursor/skills/resmate-use-case/docs/id-lifecycle.md` (NEW)
...
- **The Lifecycle Flow**:
  ...
  2. **Platform Registration**: Run `resmate tool push <name>`.
...
### 3. Create Push/Wire Document — `templates/workspace/.cursor/skills/resmate-use-case/docs/push-pull-wire.md` (NEW)
...
- **The Push-Wire-Re-Push Loop**:
  - Explain that `resmate push-all` respects the phase order but **does not** automatically inject generated IDs into dependent files.
  - Detail the step-by-step loop:
    1. Run `resmate push-all` to push all initial layers (HITL, Workflows, Tools).
    ...
    4. Run `resmate agent push <name>` to deploy the agents.
    ...
    7. Run `resmate assistant push <name>` to deploy the assistant.

### 4. Expand CLI Command Reference — `templates/workspace/.cursor/skills/resmate-use-case/docs/cli-commands.md`
...
### 5. Retarget Rules Links — `templates/workspace/.cursor/rules/resmate-use-cases.mdc`
...
- `[platform/execution-model.md](../../platform/execution-model.md)` -> `[decision-matrix.md](../skills/resmate-use-case/docs/decision-matrix.md)`
- `[react-patterns.md](../../platform/react-patterns.md)` -> `[decision-matrix.md](../skills/resmate-use-case/docs/decision-matrix.md)`
- `[tools/tool-yaml-reference.md](../../tools/tool-yaml-reference.md)` -> `[spec-tool.md](../skills/resmate-use-case/docs/spec-tool.md)`
- `[agents/agent-yaml-reference.md](../../agents/agent-yaml-reference.md)` -> `[spec-agent.md](../skills/resmate-use-case/docs/spec-agent.md)`
- `[assistants/assistant-yaml-reference.md](../../assistants/assistant-yaml-reference.md)` -> `[spec-assistant.md](../skills/resmate-use-case/docs/spec-assistant.md)`
- `[hitl/README.md](../../hitl/README.md)` -> `[spec-tool.md](../skills/resmate-use-case/docs/spec-tool.md)`
- `[workflows/README.md](../../workflows/README.md)` -> `[spec-workflow.md](../skills/resmate-use-case/docs/spec-workflow.md)`
- `[state-and-workflows.md](../../platform/state-and-workflows.md)` -> `[spec-workflow.md](../skills/resmate-use-case/docs/spec-workflow.md)`
...
- [ ] A freshly initialized workspace contains the updated `SKILL.md`, `cli-commands.md`, `AGENTS.md`, and `resmate-use-cases.mdc`.
- [ ] The new files `id-lifecycle.md` and `push-pull-wire.md` are present in the workspace's skill docs folder.
- [ ] `resmate-use-cases.mdc` contains zero broken relative links; all links resolve to valid files in the workspace.
...
2. **Verify Link Integrity**:
   ...
   - Verify that all relative links in `resmate-use-cases.mdc` and `SKILL.md` resolve correctly to existing files.
