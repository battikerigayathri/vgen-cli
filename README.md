...
|----------|---------|---------|
| macOS / Linux | `vgen-*-<platform>-<arch>.tar.gz` | `./install.sh` |
| Windows | `vgen-*-windows-x64.zip` | `powershell -ExecutionPolicy Bypass -File .\install.ps1` |

Default user-local install puts binaries in `~/.local/bin` (Unix) or `%LOCALAPPDATA%\Programs\ResMate\bin` (Windows) and templates in the co-installed `share/vgen/templates/` tree. See [docs/README-INSTALL.md](docs/README-INSTALL.md) and [docs/QUICKSTART.md](docs/QUICKSTART.md).
...
That crate is used **only** by `vgen workflow validate` for local workflow schema and semantic validation. That command does not make runtime calls to Prajna or Smriti APIs.

## Building

From `resmed_vgen-cli`:
...
| Command | Purpose |
|---------|---------|
| `vgen workspace info` | Workspace root, artifact dirs, counts, config summary |
| `vgen doctor [--offline]` | Environment and connectivity diagnostics |
| `vgen graph [--assistant <name>]` | Dependency link graph (assistants → agents → tools → HITL/workflows) |
| `vgen validate [--strict]` | Aggregate validation (graph, artifacts, workflows, secrets) |
| `vgen workflow validate <folder>` | Local workflow schema + semantics (no API) |
| `vgen push-all --dry-run [--assistant <name>]` | Preview push plan in dependency order |
...
```bash
vgen --json workspace info | jq '.ok == true'
...
| Command | Purpose |
|---------|---------|
| `vgen explain <code>` | Explain a stable error code with remediation hints |
| `vgen explain --list` | List all known error codes |
| `vgen init` | Bootstrap workspace layout and `vgen.yaml` |
| `vgen scaffold <recipe>` | Apply a recipe template (e.g. `oracle-pr`) |
| `vgen push-all --yes` | Execute batch push in dependency order (hitl → workflow → tool → agent → assistant) |
| `vgen sync` | Pull remote assistant graph including HITL configs |
| `vgen-mcp` | Stdio MCP server wrapping inspect, validate, plan, and push tools |
...
vgen init --name my-assistant
vgen scaffold oracle-pr --name my-assistant   # optional recipe
vgen doctor && vgen validate && vgen push-all --dry-run
vgen push-all --yes
```
...
Legacy [docs/CLAD-vgen-use-cases.md](docs/CLAD-vgen-use-cases.md) is deprecated. Platform monorepo `cli-context/` redirects here — see [MIGRATION.md](../resmedai-core-framework/cli-context/MIGRATION.md).
...
| Command | Purpose |
|---------|---------|
| `vgen validate --remote` | Local validation plus remote drift rules (cached; skips offline with `REMOTE_SKIPPED_OFFLINE`) |
| `vgen diff <type> <name>` | Compare local artifact vs remote (`tool`, `agent`, `assistant`, `hitl`, `workflow`) |
...
**Integration smoke** (from `resmed_vgen-cli`, against `pr-agent-v2`):
...
- **x64:** `target/x86_64-pc-windows-gnu/release/vgen.exe`, `vgen-mcp.exe`
- **x86:** `target/i686-pc-windows-gnu/release/vgen.exe`, `vgen-mcp.exe`
