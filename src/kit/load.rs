...
/// Kit version recorded in `resmate.yaml` after init.
...
/// Candidate install-time template roots (checked in order after `RESMATE_TEMPLATES_DIR`).
fn installed_template_candidates() -> Vec<PathBuf> {
    let mut candidates = Vec::new();

    if let Ok(exe) = env::current_exe() {
        if let Some(parent) = exe.parent() {
            candidates.push(parent.join("../share/resmate/templates"));
        }
    }

    if let Some(home) = dirs::home_dir() {
        candidates.push(home.join(".local/share/resmate/templates"));
    }

    #[cfg(unix)]
    {
        candidates.push(PathBuf::from("/usr/local/share/resmate/templates"));
        candidates.push(PathBuf::from("/opt/homebrew/share/resmate/templates"));
    }
...
///
/// Resolution order:
/// 1. `RESMATE_TEMPLATES_DIR` when set
...
pub fn templates_root() -> Result<PathBuf, KitError> {
    ...
    if let Ok(dir) = env::var("RESMATE_TEMPLATES_DIR") {
        let path = PathBuf::from(&dir);
        attempted.push(format!("RESMATE_TEMPLATES_DIR={}", path.display()));
        if is_valid_templates_root(&path) {
            return Ok(normalize_templates_root(path));
        }
        return Err(KitError::TemplatesNotFound(format!(
            "RESMATE_TEMPLATES_DIR={} does not contain workspace/",
            path.display()
        )));
    }
    ...
    Err(KitError::TemplatesNotFound(format!(
        "Authoring kit templates not found. Attempted:\n  {}\n\
         Re-run install.sh / install.ps1 or set RESMATE_TEMPLATES_DIR.",
        attempted.join("\n  ")
    )))
}
...
mod tests {
    ...
    fn templates_root_finds_manifest_dir_in_dev() {
        ...
        assert!(root.join("seed/resmate.yaml.tmpl").is_file());
    }
    ...
    fn templates_root_respects_env_override() {
        ...
        unsafe {
            env::set_var("RESMATE_TEMPLATES_DIR", &manifest);
        }
        let root = templates_root().expect("RESMATE_TEMPLATES_DIR override should work");
        assert_eq!(root, normalize_templates_root(manifest.clone()));
        unsafe {
            env::remove_var("RESMATE_TEMPLATES_DIR");
        }
    }

    #[test]
    fn templates_root_env_must_contain_workspace() {
        let tmp = env::temp_dir().join(format!("resmate-no-workspace-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
        unsafe {
            env::set_var("RESMATE_TEMPLATES_DIR", &tmp);
        }
