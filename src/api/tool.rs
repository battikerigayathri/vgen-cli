...
pub async fn test_faas_tool(payload: &Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
    let cfg = config::load_config().map_err(|e| e.to_string())?;
    let api_key = cfg
        .api_key
        .as_deref()
    ...
    let client = http_client::build_client();
    let req = client.post(&url).json(payload).header("Content-Type", "application/json");
    let req = http_client::add_auth_headers(req, api_key)?;
    let res = req.send().await?;
