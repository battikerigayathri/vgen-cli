...
The `vgen` CLI utilizes local environment variables to resolve its active configuration and authenticate requests against platform APIs.
...
## 2. Platform Secret Mapping: `vgen.yaml`

To bridge the gap between local environment values and remote orchestrator access, developers define mapping configurations inside the `env_mappings` array block of the workspace manifest (`vgen.yaml`).
...
## 4. Bulk Secret Deployment: `vgen env push`

Deploy local environment variables mapped in `vgen.yaml` to Smriti's credentials store.
...
2.  **No Decryption in CLI**: The `vgen` CLI strictly forbids retrieving, showing, or decrypting secrets stored on the platform. Commands like `vgen env get` or `vgen env show` do not exist.
