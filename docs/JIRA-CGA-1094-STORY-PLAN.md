...
|-------|-------|
| **Story points** | 5 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-1`, `pr1`, `foundation` |
...
|-------|-------|
| **Story points** | 3 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-1`, `pr2`, `workspace` |
| **Depends on** | P1-1 |
| **Repo** | `resmed_vgen-cli` |

**Description**

Workspace detection and environment diagnostics: `vgen workspace info` and `vgen doctor [--offline]`. Honors `VGEN_*_DIR` overrides. Doctor checks: workspace_root, config_load, api_key_set, jwt_secret (warn), connectivity (skip offline), artifact_dirs_exist.
...
|-------|-------|
| **Story points** | 5 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-1`, `pr3`, `graph` |
| **Depends on** | P1-2 |
| **Repo** | `resmed_vgen-cli` |

**Description**

`vgen graph [--json] [--assistant <name>]` — assistant → agent → tool → hitl → workflow graph with broken-ref detection and orphan listing. Move `extract_workflow_slug` to `specs/assistant.rs`; add discovery helpers and `ResourceIndex`.
...
|-------|-------|
| **Story points** | 5 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-1`, `pr4`, `workflow`, `smriti_client` |
| **Depends on** | P1-1 (parallel with P1-2, P1-3 after P1-1) |
| **Repo** | `resmed_vgen-cli`, `resmedai-core-framework` (smriti feature gate) |

**Description**

`vgen workflow validate <folder> [--json]` — local schema + semantic validation via `smriti_client` `WorkflowDefinitionLoader`. Exit `3` on failure; no network. Prefer git dep with `validate` feature (`default-features = false`).
...
|-------|-------|
| **Story points** | 8 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-1`, `pr5`, `validation` |
| **Depends on** | P1-3, P1-4 |
| **Repo** | `resmed_vgen-cli` |

**Description**

`vgen validate [--json] [--strict] [--offline]` — graph integrity, artifact lint, workflow validate, secrets grep, push readiness. Reuses `graph::build_graph` and workflow validate internally.
...
|-------|-------|
| **Story points** | 3 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-1`, `pr6`, `push-plan` |
| **Depends on** | P1-5 |
| **Repo** | `resmed_vgen-cli`, `resmedai-core-framework` (cli-context) |

**Description**

`vgen push-all --dry-run [--json] [--assistant <name>]` — ordered push plan (hitl → workflow → tool → agent → assistant), Create/Update detection, validation blockers. No HTTP traffic.
...
|-------|-------|
| **Story points** | 8 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-2`, `mcp` |
...
- [ ] Document setup in `resmed_vgen-cli` README and `cli-context`
...
### Story P2-2: vgen init / scaffold + vgen.yaml manifest

| Field | Value |
|-------|-------|
| **Story points** | 5 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-2`, `scaffold` |
| **Depends on** | P1-1 |
| **Repo** | `resmed_vgen-cli` |

**Description**

`vgen init` and `vgen scaffold <recipe>` generate standard workspace structure. Optional `vgen.yaml` manifest defining artifact paths, recipes, and defaults (replaces implicit env-var-only discovery over time).

**Acceptance criteria**

- [ ] `vgen init` creates tools/, agents/, assistants/, hitl/, workflows/ skeleton
- [ ] `vgen scaffold oracle-pr` (or similar) produces Oracle PR starter layout
- [ ] `vgen.yaml` schema documented; workspace info reads manifest when present
...
|-------|-------|
| **Story points** | 8 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-2`, `push-plan` |
| **Depends on** | P1-6 |
| **Repo** | `resmed_vgen-cli` |
...
- [ ] `vgen push-all` (without `--dry-run`) executes hitl → workflow → tool → agent → assistant order
...
### Story P2-4: Fix vgen sync to pull HITL

| Field | Value |
|-------|-------|
| **Story points** | 3 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-2`, `sync`, `bugfix` |
| **Depends on** | — (can parallel Phase 1) |
| **Repo** | `resmed_vgen-cli` |

**Description**

Documentation states `vgen sync` pulls HITL artifacts; implementation does not. Align code with docs (or update docs if intentional — prefer fixing code).

**Acceptance criteria**

- [ ] `vgen sync` pulls HITL definitions when IDs exist remotely
...
### Story P2-5: vgen explain error codes

| Field | Value |
|-------|-------|
| **Story points** | 3 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-2`, `errors` |
| **Depends on** | P1-1 |
| **Repo** | `resmed_vgen-cli` |

**Description**

`vgen explain <error-code>` — human and JSON explanation of stable error codes from `docs/errors/`. Helps agents and developers remediate without reading source.

**Acceptance criteria**

- [ ] `vgen explain BROKEN_AGENT_REF` prints description, severity, remediation hints
- [ ] `vgen --json explain WORKFLOW_SCHEMA_INVALID` returns structured payload
- [ ] All codes in `docs/errors/*.md` are discoverable via `vgen explain --list`
...
|-------|-------|
| **Story points** | 2 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-2`, `documentation` |
...
|-------|-------|
| **Story points** | 5 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-3`, `validation` |
| **Depends on** | P1-5 |
| **Repo** | `resmed_vgen-cli` |

**Description**

Extend `vgen validate` with optional `--remote` checks: verify referenced platform IDs exist via authenticated API lookup (tools, agents, assistants, hitl, workflows).

**Acceptance criteria**

- [ ] `vgen validate --remote` flags IDs not found on platform
...
### Story P3-3: vgen diff before push

| Field | Value |
|-------|-------|
| **Story points** | 5 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-3`, `diff` |
| **Depends on** | P1-5, P1-6 |
| **Repo** | `resmed_vgen-cli` |

**Description**

`vgen diff [<resource>]` — show local vs remote diff before push. JSON output for agent consumption.

**Acceptance criteria**

- [ ] `vgen diff tool roc-search-users` shows field-level delta vs platform
...
|-------|-------|
| **Story points** | 5 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-3`, `workflow`, `smriti_client` |
| **Depends on** | P1-4 |
| **Repo** | `resmed_vgen-cli` |
...
### Story P3-5: vgen-cli CLAD documentation full replacement

| Field | Value |
|-------|-------|
| **Story points** | 3 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-3`, `documentation`, `clad` |
| **Depends on** | Phase 1–2 complete |
| **Repo** | `resmed_vgen-cli`, `resmedai-core-framework` |

**Description**

Replace legacy CLAD doc (`docs/CLAD-vgen-use-cases.md`) with current agent-authoring model: JSON CLI, validate/graph/push-all, cli-context as source of truth.
...
|-------|-------|
| **Story points** | 8 |
| **Labels** | `vgen-cli`, `vgen`, `agent-authoring`, `phase-3`, `ci`, `testing` |
| **Depends on** | P1-5 |
| **Repo** | `resmed_vgen-cli` |
...
- [ ] CI workflow runs on PR to `resmed_vgen-cli`
...
| [CGA-1102](https://resmedglobal.atlassian.net/browse/CGA-1102) | init/scaffold + vgen.yaml | 2 | 5 | CGA-1095 |
...
| [CGA-1108](https://resmedglobal.atlassian.net/browse/CGA-1108) | vgen diff | 3 | 5 | CGA-1099, CGA-1100 |
...
additional_fields:
  ...
  labels: [vgen-cli, vgen, agent-authoring, phase-N, ...]
```
